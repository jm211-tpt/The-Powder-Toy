...
static void base_reactions(int base1_id, UPDATE_FUNC_ARGS)
{
	for (int rx = -1; rx <= 1; rx++)
	{
		for (int ry = -1; ry <= 1; ry++)
		{
			if (rx || ry)
			{
				...
				// BASE + GOO -> OIL + WAX  (note, base dissolves ptnm if life>=10)
				if (rt == PT_GOO)
				{
					sim->part_change_type(ID(r), x + rx, y + ry, PT_WAX);
					sim->part_change_type(base1_id, x, y, PT_OIL);
					return;
				}
			}
		}
...
static int update(UPDATE_FUNC_ARGS)
{
	...
	int base1_id = -1; // same but for base
	...
	if (base1_id >= 0)
	{
		base_reactions(base1_id, UPDATE_FUNC_SUBCALL_ARGS);
	}
